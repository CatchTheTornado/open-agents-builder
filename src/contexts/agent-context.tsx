import {
  AdminApiClient,
  ApiEncryptionConfig,
} from "@/data/client/admin-api-client";
import {
  Agent,
  DataLoadingStatus,
  Session,
...
import {
  AgentApiClient,
  DeleteAgentResponse,
} from "@/data/client/agent-api-client";
...
import {
  ...
  ResultApiClient,
} from "@/data/client/result-api-client";
import {
  DeleteSessionResponse,
  SessionApiClient,
} from "@/data/client/session-api-client";
...
interface AgentContextType {
  ...
  agentResults: (
    agentId: string,
    { limit, offset, orderBy, query }: PaginatedQuery
...
export const AgentProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [results, setResults] = useState<PaginatedResult<Result[]>>({
    rows: [],
    total: 0,
    limit: 0,
    offset: 0,
  ...
  const [sessions, setSessions] = useState<PaginatedResult<Session[]>>({
    rows: [],
    total: 0,
    limit: 0,
    offset: 0,
  ...
  const [loaderStatus, setLoaderStatus] = useState<DataLoadingStatus>(
      ...
      if (template.extra) {
        ...
        if (templateMeta["importProductsFromUrl"]) {
          try {
            ...
            const examplesArrayBuffer = await apiClient.getArrayBuffer(
              templateMeta["importProductsFromUrl"]
            );
            await productContext.importProducts(
              examplesArrayBuffer as ArrayBuffer
            );
        ...
        if (templateMeta["importAttachmentsFromUrl"]) {
          try {
            ...
            const examplesArrayBuffer = await apiClient.getArrayBuffer(
              templateMeta["importAttachmentsFromUrl"]
            );
            await attContext.importAttachments(
              examplesArrayBuffer as ArrayBuffer
            );
  ...
  const deleteSession = async (
    ...
    if (resp.status === 200) {
      setSessions({
        ...sessions,
        rows: sessions.rows.filter((pr) => pr.id !== session.id),
      });
    }
  ...
  const deleteResult = async (
    ...
    if (resp.status === 200) {
      setResults({
        ...results,
        rows: results.rows.filter((pr) => pr.sessionId !== result.sessionId),
      });
    }
  ...
  const agentSessions = async (
    ...
    const client = await setupApiClient();
    const response = await client.sessions(agentId, {
      limit,
      offset,
      orderBy,
      query,
  ...
  const listAgents = async (currentAgentId?: string): Promise<Agent[]> => {
    ...
    try {
      ...
      const fetchedAgents = apiResponse.map((folderDTO: AgentDTO) =>
        Agent.fromDTO(folderDTO)
      );
      setAgents(fetchedAgents);
      setLoaderStatus(DataLoadingStatus.Success);
      fetchedAgents.push(newAgent());

      let defaultAgent: Agent | null =
        fetchedAgents.length > 0
          ? fetchedAgents[fetchedAgents.length - 1]
          : null;
      if (!current) {
      ...
      if (currentAgentId) {
        // force to switch to this agent
        defaultAgent =
          fetchedAgents.find((f) => f.id === currentAgentId) || null;
      }
  ...
  const exportAgent = (agent: Agent) => {
    ...
    const file = new Blob([JSON.stringify(agent)], {
  ...
  const exportSingleResult = (result: Result) => {
    ...
    const mimeType =
      resultFormat === "markdown" ? "text/markdown" : "application/json";
  ...
  const exportResults = async (agent: Agent) => {
    if (agent.id && agent.id !== "new") {
      ...
      const a = document.createElement("a");
      const file = new Blob([(await apiClient.export(agent.id)) ?? ""], {
